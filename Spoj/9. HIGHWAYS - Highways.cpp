/*
	...
	Carnet: 1710303
	Nick: jmlt_undercode
	Problema: SPOJ 9. HIGHWAYS - Highways
	
	NOTA IMPORTANTE:
	Comente muchos bloques de codigo/variables, etc que no se usen para solventar el problema. 
	En lo personal ayuda a ubicarme dentro de las 800 lineas de codigo que lleva la libreria. 
	Segundo creo que ayuda al lector a ubicarse de la misma forma.

	** Idea General para el problema: **

	Aplicar el algoritmo de Dijkstra para determinar el camino mas corto entre dos ciudades.
	Sin embargo, mas abajo estan descritas dos formas de aplicar Dijsktra, una con set y otra
	con Priority Queue. Se probo con sets bajo la estructura NODE y el error dado era TLE. 
	Con aplicacion de Priority Queue bajo la estructura pair se pudo solventar el problema.

	** Actualizacion de Libreria/Class: **
	1. Se agrega la redefinicion de operador MENOR QUE y MAYOR QUE para clase NODE.
	2. Creacion de nueva estrucura ARCO para almacenar sus extremos y costo/peso.
	3. Mejora de la lista de adyacencia de la clase Graph, ahora admite pesos de arcos. 
	4. Mejora de funcion "add_arc" para agregar costos a los arcos, va de la mano con la
	actualizacion numero 3.
	5. Se agrega la funcion "DijsktraSET" para calcular camino de costo minimo desde una 
	raiz. Esta implementacion usa Sets bajo la estructura NODE.
	6. Se agregan nuevas estructuras a la clase Graph para el uso de Dijsktra. 
	7. Se agrega la funcion "DijsktraPQ" para calcular camino de costo minimo desde una 
	raiz. Esta implementacion usa Priority Queue bajo la estructura pair.
...
public:
	...
	int weight;	// Peso del arco.
	ARC() : n(-1), ady(-1), weight(-1){}
	ARC(int _n, int _ady, int _w) : n(_n), ady(_ady), weight(_w){}
};
...
const int MaxN = 1e5 +5;	// Numero maximo de nodos.(Ciudades)
int T; 				// Numero de casos.
int N, M;			// Numero de nodos y de arcos.(Ciudades y carreteras)
int S, F;			// Nodo inicial(Start) y nodo de llegada(Final).
int A, B, W;			// Arco (A,B) con peso/tiempo W. 
...
public:
	...
	*/
	...
	void add_arc(int n1, int n2, int w) {
		if ( type_graph == 1 ) {
	...
	vi print_vb(vb a){
		vi r;
		for (int k = 1; k < a.size(); k++) { if(a[k]) r.pb(k); }
		return r;
	}
};
...
int main() {
	...
	while(T--){
		...
		// Buscamos el camino de menor costo desde la raiz S.
		G.DijkstraPQ(S);
		
		// Mostrar resultado por pantalla.
		int r = G.Nodes[F].min_cost;
		if (r == INF){
			cout << "NONE" << endl;
		} else {
			cout << r << endl;

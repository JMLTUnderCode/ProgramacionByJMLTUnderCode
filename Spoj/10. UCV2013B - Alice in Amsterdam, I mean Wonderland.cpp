/*
	...
	Carnet: 1710303
	Nick: jmlt_undercode
	Problema: SPOJ 10. UCV2013B - Alice in Amsterdam, I mean Wonderland
	
	** Idea General para el problema: **
	
	Estamos frente a un problema donde debemos determinar caminos de costo minimo y 
	detectar los arcos que forman parte de un ciclo negativo.

	Para solventar el problema se muestran dos formas. 
	La primera es usando el Algoritmo de Floyd-Warshall para determinar caminos de costo
	minimo entre todo par de nodos con una modificacion adicional que permite determinar
	arcos que pertenecen a ciclos de costo negativo. 
	Esta solucion tiene TIME 0.80 y MEM 5.2M en SPOJ.

	La segunda es usando el Algoritmo de Bellman-Ford para determinar caminos de costo
	minimo entre una raiz y todos los nodos del grafo. Para este problema se aplica
	el algoritmo N veces (numero de nodos). 
	Esta solucion tiene TIME 1.42 y MEM 5.4M en SPOJ.

	** Actualizacion de Libreria/Class: **
	1. Ligera modificacion en clase NODE y funcion add_arc, variable peso cambiada a
	tipo LONG LONG INT.
	2. Inclusion de funciones/algoritmos Floyd-Warshall y Bellman-Ford.
	3. Inclusion de variables y tipo de datos en la estructura Graph para el
	funcionamiento de los algoritmos incluidos en (2).
...
int N;				// Numero de nodos.(Monumentos)
lli K; 				// Distancia entre monumentos;
int Q;				// Numero de de querys.(Solicites)
string monuments[MaxN];		// Array para almacenar los nombres de los monumenetos;
...
public:
	...
	priority_queue<pi, vector<pi>, greater<pi>> P_Q;// Priotiry Queue usada en Dijsktra.
	*/
	...
	/*void DijkstraPQ(int root){
		...
		while( !P_Q.empty() ){
			...
			if(Nodes[v.second].visited) continue;
			// Verificamos que el costo actual del nodo sacado del set sea el mismo
			// ya calculado.
			int curr_c = Nodes[v.second].min_cost;

			// Relajamos los arcos para todos los adyacentes del nodo actual.
			for(auto it : lists[v.second]){
				int new_c = curr_c + it.weight;
				if( Nodes[it.ady].min_cost <= new_c ) continue;

				Nodes[it.ady].min_cost = new_c;
				P_Q.push(make_pair(new_c, it.ady));
			}
			Nodes[v.second].visited = 1;
		}
	}*/
...
int main() {
	...
	while(cin >> N && N != 0){
		...
		FOR1(i, N){
			...
			FOR1(j, N) { 
				cin >> K;
				// Agregamos los arcos existentes, lo que sean igual a 0 se ignoran.
				if (i != j && K == 0) continue;

				// En caso de tener un peso mayor a 0 siendo un arco de un nodo a si mismo
				// entonces cambiamos a 0 dicho peso.
				if (K > 0 && i == j) K = 0;

				// En caso de ser un peso negativo para un arco de un nodo a si mismo
				// lo cambiamos a -INF, pues ya indica ciclo infinito.
				if( K < 0 && i == j) K = -INF;
		...
		// Se aplica solucion 1: Floyd-Warshall TIME 0.80, MEM 5.2M
		G.Floyd_Warshall();
		// Se aplica solucion 2: Bellman_Ford TIME 1.42, MEM 5.4M
		//FOR1(n,N) G.Bellman_Ford(n);
		...
		while(Q--){
			...
			if( G.dist[q1][q2] <= -INF ) {
				...
				cout << monuments[q1] << "-" << monuments[q2] << " ";
				if (G.dist[q1][q2] == INF) {
					cout << "NOT REACHABLE" << endl;
				} else {
					cout << G.dist[q1][q2] << endl;
				}
			}

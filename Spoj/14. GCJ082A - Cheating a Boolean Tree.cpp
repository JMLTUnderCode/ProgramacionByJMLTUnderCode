/*
	...
	Carnet: 1710303
	Nick: jmlt_undercode
	Problema: SPOJ 14. GCJ082A - Cheating a Boolean Tree

	** Idea General para el problema: **
		
	Con una idea de Programacion Dinamica para arboles booleanos, se plantea una busqueda
	recursiva para los distintos casos de prueba. Se define una funcion DinP(n, g) para 
	cada nodo n del arbol como el menor numero de puertas a cambiar para obtener como objetivo
	g. Se plantean dos casos bases y uno recursivo. 
	Casos bases:
	1. En caso de nodo n ser una hoja y cumplir con la meta g, entonces no requiere cambio de 
	puerta, por tanto return 0.
	2. En caso de nodo n ser una hoja y NO cumplir con la meta g, entonces es imposible realizar
	cambios, por tanto return INFINITO.
	3. Se realizar calculan los 4 casos posibles del nodo actual n. Se define L y R como hijos
	de n, siendo L hijo izquierdo y R hijo derecho. Se calcula:
	- Llamar a los dos hijos con objetivo 0: DinP(L, 0) y DinP(R, 0)
	- Llamar a los dos hijos con objetivo 1: DinP(L, 1) y DinP(R, 1)
	Luego, los respectivos casos segun el tipo de puerta(AND, OR) se forman calculando el minimo
	entre los datos calculados de L y R para el tipo de meta a cumplir. A esto tambien se debe
	incluir el caso de ser posible el cambio de puerta propio del nodo actual, si es asi
	entonces se debe tratar al inverso, es decir si la puerta actual es AND entonces se calcula
	los minimos como si fuera de tipo OR, si es OR entonces como si fuese AND.
	
	** Actualizacion de Libreria/Class: **
	1. Inclusion de la funcion DinP de uso recursivo con implementacion de Programacion Dinamica
	para el calcular el numero minimo de cambios en un arbol booleado que debe tener un nodo en
	el camino de conseguir determinado valor la raiz.
	2. Inclusion de una rutina que permite determinar el minimo de tres numeros.
	3. Cambio de nombre y mejora en descripcion de la funcion "FastIO_ReadFile" que anteriormente 
	era llamanda "init_code".
*/
...
const int MaxM = 10005;		// Numero maximo de nodos.
int N;				// Numero de casos de prueba.
int M; 				// Numero de nodos.
int goal;			// Valor objetivo del nodo raiz del arbol buleano.
vlli DP(MaxM, 0);		// Vector de Programacion Dinamica para calculados de valor de nodo.
vi type_of_door(MaxM, 0);	// Vector que indica el tipo de puerta de un nodo, si es AND o OR.	
vi change_doors(MaxM, 0); 	// Vector que indica si un nodo es posible cambiar la puerta(AND, OR)
vi is_leaf(MaxM, 0);		// Vector que indica si un nodo es hoja o no.
...
int main() {
	...
	while (cases - 1 < N) {
		...
		// Leemos los nodos tipo PADRE.
		FOR(m, 0, (M-1)/2){
			cin >> type_of_door[m] >> change_doors[m];
			DP[m] = INF;
			is_leaf[m] = 0;
		}

		// Leemos los nodos tipo HOJA.
		FORE(m, (M-1)/2, M){
			cin >> DP[m];
			is_leaf[m] = 1;
		}
		...
		// Mostrar resultado.
		cout << "Case #" << cases << ": ";
		solution = G.DinP(0, goal);
		if(solution < INF) cout << solution << endl;

/*
	...
	Carnet: 1710303
	Nick: jmlt_undercode
	Problema: SPOJ 13. THECODE - Subset with all Digits

	** Idea General para el problema: **
	
	Primeramente se dejan dos enlaces a dos algoritmos que facilitan el entendimiento
	del codido escrito para este problema, como tambien la recomendacion que deja
	el propio problema de SPOJ leer sobre "Set cover" y un ejemplo de Programacion
	dinamica con bitmask.

	Algoritmo para contar bits, Brian Kernighan. No es usado en el codigo resolvente
	pero sirve como una herramienta futura.
	https://www.techiedelight.com/es/brian-kernighans-algorithm-count-set-bits-integer/
	
	Articulo para entender la programacion dinamica con bitmask.
	https://www.hackerearth.com/practice/algorithms/dynamic-programming/bit-masking/tutorial/
	
	Articulo de Wikipedia para aprender sobre el problema de Conjunto Covertor.
	https://en.wikipedia.org/wiki/Set_cover_problem

	Articulo de ejemplo DP Bitmasks.
	https://www.geeksforgeeks.org/bitmasking-and-dynamic-programming-set-1-count-ways-to-assign-unique-cap-to-every-person/

	Inicialmente se convierte todo "numero"(string de 0's a 9's) en una representacion
	de bitmask. Se cubre el caso de guardar una mascara una sola vez. es decir
	si se otorga "78" y "87" a nivel de mask es lo mismo, por tanto skip el segundo input,
	asi mismo el caso de obtener la mask que cubre de 0 a 9 directamente retornar 1.
	Luego se procede con al programacion dinamica por cada una de las 
	bitmask leidas en caso cualquier otro caso. 
	Esta DP(Dinamic Programation) sigue la idea vista en el articulo mencionado arriba.
	Se debe formular como una problema de Asignacion con costo constante en 1 ya que
	el se debe buscar un grupo de elementos tal que al unirlos, mediante mascaras 
	obtengamos (2^10)-1 = Hexa 1023 = Bin 11 1111 1111, como no existe preferencia
	dado que todos las mask pueden ser elegidas sin problema alguno para tener la menor
	numero de mask tal que obtengamos Bin 11 1111 1111, entonces el costo se setea
	por defecto en 1. Luego se itera por todas las mascaras buscando la minima 
	asignacion de mascaras(elementos) tal que se obtenga la Mask Bin 11 1111 1111.
	El paso de encontrar la minima asignacion es como en un paso de "Relajacion"
	vista en otros algoritmos, Dijsktra, Bellmand-Ford y Floyd-Warshall por ejemplo.

	** Actualizacion de Libreria/Class: **
	1. Inclusion del algoritmo de Brian Kernighan para determinar cantidad de bits
	encendidos en un numero.
	2. Inclusion del algoritmo de Programacion Dinamica para problema de Asignacion
	con el uso de bitsmask.
	3. Inclusion de dos directivas FOR adicionales, FORE y FORI.
...
const int MaxN = 1e3+2;	// Numero maximo de nodos.
int T;			// Numero de casos de prueba.
int N;			// Cantidad de numeros dados.
int D; 			// Cantidad de digitos en el numero dado.
string number;		// Array de caracteres representativo de un numero muy grande.
...
int main() {
	...
	while (T--) {
		...
		mask_complete = false;
		vi masks_list(N, 0); // Lista de mascaras.
		vi vis(1025, 0);     // Vector de visitados para guarda una unica mask.
		num_mask = 0;	     // Contador de mascaras unicas.
		FOR(k, 0, N){
			cin >> number;	
			mask = 0;
			// Transformacion de string s bitmask.
			FOR(x, 0, D) {
				mask = mask | (1 << (number[x] - '0'));
			}
			if(vis[mask]) continue;
			vis[mask] = 1;
			masks_list[num_mask++] = mask;
			if(mask == 1023) mask_complete = true; 
		}
 		
 		// Verificamos si existe una mascara completa.
 		if(mask_complete) cout << "1\n";
 		else cout << G.DP_assign(masks_list, num_mask) << endl;
	}

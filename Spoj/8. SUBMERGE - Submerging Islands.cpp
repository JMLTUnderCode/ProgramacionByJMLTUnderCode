/*
	...
	Carnet: 1710303
	Nick: jmlt_undercode
	Problema: SPOJ 8. SUBMERGE - Submerging Islands
	
	** Idea General para el problema: **
	
	Se quiere determinar puntos de articulacion para un grafo conexo dado. Al final que en el
	problema "EC_P - Critical Edges" se modifico la funcion Tarjan para que calculara ahora
	dichos puntos de articulacion.

	Cuando se cumple la funcion de que "disc[root] <= low[ady] && child_tracker[root] > 1" 
	siendo "ady" un nodo adyacente a "root" y el array child_tracker un contador de hijos para
	cuyo indice es el nodo, se tiene que si el discovery de la raiz es menor o igual al low
	del nodo adyacente y dicha raiz tiene mas de un hijo entonce se considera punto de articulacion.

	Cabe destacar que este analisis y dicha condicion se realiza sobre el arbol DFS que proporcionar
	el mismo algoritmo Tarjan en su ejecucion. Dicho esto, con esto hemos cubierto el caso de nodos
	hojas en el arbol dfs y el caso del nodo raiz, pues luego de ejecutar Tarjan debemos verificar
	que el numero de hijos de la raiz sea mayor a 1 para considerarlo punto de articulacion.

	** Actualizacion de Libreria/Class: **
	
	1. Comentado de algunas variables de class NODE y en funcion add_arcs para reducir 
	peso a esta solucion.
	2. Modificacion de la funcion "Tarjan", ahora calcula C.F.C./C.C, puentes y puntos de 
	articulacion.
	3. Agregado de funcion "print_vb" para extraer de un vector de booleanos aquellos indices
	con valor true. Esto sirve para extraer los nodos que son punto de articulacion en el calculo
	del algoritmo Tarjan.	
*/
...
const int MaxN= 1e4 + 5;	// Numero maximo de nodos(Ciudades)
const int MaxM= 1e5 + 5;	// Numero maximo de arcos(Carreteras)
int N, M; 			// Numero de nodos y de arcos. (Islas y Puentes)
int U, V;			// Extremos de un arco, nodo U conecta con nodo V. (Islas)
...
int main() {
	...
	while(true){
		...
		// Aplicacion de Tarjan para el calculo de puntos de articulacion.
		G.parent_tracker[1] = 1;
		G.Tarjan(1);

		// Verificamos si el nodo raiz tiene mas de 1 hijo en el arbol DFS entonce 
		// se considera punto de articulacion.
		G.articulation_points[1] = G.child_tracker[1] > 1;

		// Impresion de resultados.
		result = G.print_vb(G.articulation_points);
		cout << result.size();
	}

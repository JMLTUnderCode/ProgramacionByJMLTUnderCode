/*
	...
	Carnet: 1710303
	Nick: jmlt_undercode
	Problema: SPOJ 12. BTCK - A problem of Backtracking

	** Idea General para el problema: **
	
	La solucion se basa en un backtraking mediante una mascara de bits. Mientras obtengamos
	un acumulador <= K (Siendo k la meta) retornamos la permutacion. 
	
	** Actualizacion de Libreria/Class: **
	
	1. Actualizacion de la direcetiva FOR, se pasa a triple argumento donde el segundo arg
	indica desde donde comienza la iteracion.
...
const int MaxN = 1e3;	// Numero maximo de nodos.
int T;			// Numero de casos de prueba.
ll N[10];		// Numero de N's.
ll K; 			// Resultado objetivo.
vi solution(10, -1);	// Vector solucion que contiene la permutacion respuesta.
...
public:
	...
	*/
	...
	bool find_permutation(long long A, int mask, int cnt){
		// Verificamos que hemos usados todos loa a_i y que A < K+1.
		if(mask == (1 << 10) - 1 && A < K+1) return true;

		FOR(i, 0, 10){
			if((mask & (1 << i)) == 0){
				// Verificamos antes de llamar recursivamente que la suma no
				// sobrepase a K.
				if(A + N[cnt]*i > K) return false;

				// Guardamos solucion.
				solution[cnt] = i;

				// Llamamos con la modificacion de mask, acumulador y el contador
				// de bits usados.
...
int main() {
	...
	while(T--){
		...
		// Buscamos la permutacion tal que Acumulador <= K
		if(G.find_permutation(0LL, 0, 0)) G.print_vi(solution);
		else cout << "-1\n";
	}

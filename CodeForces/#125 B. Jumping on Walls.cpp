/*
	...
	Carnet: 1710303
	Nick: JMLTUnderCode
	Problema: Codeforces Round 125 (Div. 1) B. Jumping on Walls

	** Idea General para el problema: **
	
	Se busca usar la idea de un DFS para dos "grafos abstractos". Dichos grafos se ven como
	un string compuesto por "-"(zona segura) y "x"(no segura) poder hacer el recorrido DFS/BFS.
	En intentos anteriores se tuvo problemas de memoria, por tanto se deja de usar la clase
	nodo para este problema y solamente un recorrido DFS de forma estrategica entre los 
	"grafos abstractos"(strings).
	Se realiza de forma recursiva en tres fases, llamada de salto hacia la pared opuesta 
	usando el K, llamada para subir en la misma pared y para bajar en la misma pared.
	De esta forma siempre se prioriza saltar en paso k entre paredes hasta llegar a la meta.
	En cada llamada se sube el contador de "water" que en caso de realizar una accion, el
	nivel del agua concuerda con la del personaje entonces no existe camino. Sols: Accepted.

	Inicialmente se tuvo primer intento(approach) de setear todos los arcos posibles usando 
	el K respectivo, es decir usando los movimientos posibles del ninja, el k y las condiciones
	de las paredes. Ej
	 N = 7 y K = 3
	 Left     Right
	     (15)
	...
	Arcos posibles(movimientos del ninja):
	(1 -> 2 ) (2 -> 1) (2 -> 3) (3 -> 2) (1 -> 11) 
	(11 -> 10) (10 -> 11) (10 -> 6) 
	(5 -> 15) (6 -> 15) 
	(14 -> 15)

	El valor 15 = 2*N + 1 esto simula la salida del cañon, todo camino que lleve desde 1 hasta 15
	entonces es valido. Ej. 1 -> 11 -> 10 -> 6 -> 15 (Return YES).

	Para esta implementacion se tuvo "Memory limit exceeded on test 20" debido al peso de la 
	recursion en llamadas de grafos extensos.

	** Actualizacion de Libreria/Class: **

	1. Inclusion de dos vectores "visitados1" y "visitados2" para solucionar el problema.
	2. Uso de la funcion "path_between" pero modificada en grandes rasgos por tanto se
	cambio de nombre a "path_between_recur".
...
const int MaxN = 1e5+5;		// Numero maximo de nodos.(Puntos)
int N;				// Numero de nodos.(Algura del risco)
int K;				// Altura del salto del ninja.
char w_l[MaxN];			// Pared izquierda.
char w_r[MaxN];			// Pared Derecha.
vi water(MaxN, 0);		// Vector de que marca si hay agua en la zona o no.
...
int main() {
	...
	// Llamado de funcion para encontrar el camino hacia la salida del cañon.
	if( G.path_between_recur(1, 0, 0) ) cout << "YES" << endl;
	else cout << "NO" << endl;

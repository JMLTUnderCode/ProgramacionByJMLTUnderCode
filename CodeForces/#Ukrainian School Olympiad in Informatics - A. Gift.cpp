/*
	...
	Carnet: 1710303
	Nick: JMLTUnderCode
	Problema: All-Ukrainian School Olympiad in Informatics - A. Gift
 
	** Idea General para el problema: **
	
 	Para solventar este problema se busca el uso del Algoritmo de Kruskal con la modificacion
 	de ir creando y organizando un MST en funcion de los pesos. Inicialmente se tiene un peso
 	G(Gold) del cual se ordenan los arcos de menor a mayor. Luego por cada uno de los arcos 
 	con peso oro se va agregando el menor arco(peso oro) a un nuevo MST que se debe organizar
 	en funcion del peso S(plata) esta vez de de mayor a menor.

 	La idea es obtener por cada arco nuevo agregado al nuevo MST la minima cantidad de oro y la
 	maxima cantidad de plata tal que satisfaga g_i <= a y s_i <= b del problema.
	
	** Actualizacion de Libreria/Class: **
	1. Inclusion del algoritmo Kruskal para generar un minimum spanning tree con leves 
	modificaciones sobre 2 pesos en arcos.
	2. Modificacion menor del algoritmo DSU en la parte de "find_set".
	3. Inclusion de deficiones de operador "menor que" y "mayor que" en class ARC.
...
const lli INF = 9223372036854775807LL;
...
public:
	...
	*/
	...
	NODE(){
		n = 0; visited = 0;
		//range = 0; ex_grade = 0; min_cost = 0; in_grade = 0;level = 0;
...
public:
	...
	lli g_cost;	// Tipo 2 de peso en el arco.
	lli s_cost;	// Tipo 1 de peso en el arco.
 
	ARC() : n(-1), ady(-1), g_cost(-1), s_cost(-1){}
	ARC(int _n, int _ady, lli gc, lli sc) : n(_n), ady(_ady), g_cost(gc), s_cost(sc){}
 
	bool operator>(const ARC &otherArc) const {
		return g_cost > otherArc.g_cost;
	}
	bool operator<(const ARC &otherArc) const {
		return g_cost < otherArc.g_cost;
	}
...
const int MaxN = 205;	  // Numero maximo de nodos.
const int MaxM = 5e5 + 5; // Numero maximo de arcos.
int N;			// Numero de nodos/ciudades.
int M;			// Numero de arcos/carreteras.
int Gold, Silver;	// Numero de Oro y Plata en tugriks.
...
public:
	...
	va Edges;		// Vector de arcos tipos class ARC.
	/*
	...
	*/
	...
	void add_arc_v2(int n1, int n2, lli gc, lli sc) {
		if ( type_graph == 1 ) {
			lists_a[n1].pb(ARC(n1, n2, gc, sc));
			Edges.pb(ARC(n1, n2, gc, sc));
			if(Nodes[n1].n == 0 && n1 != 0) Nodes[n1] = NODE (n1, 0);
			//Nodes[n1].ex_grade++;
			if(Nodes[n2].n == 0 && n2 != 0) Nodes[n2] = NODE (n2, 0);
			//Nodes[n2].in_grade++;
	...
	int find_set(int v) {
		if(v == parent_tracker[v]) return parent_tracker[v];
		else return find_set(parent_tracker[v]);
	}
 
	void union_sets(int a, int b) {
		if (a != b) parent_tracker[a] = parent_tracker[b];
	}
	...
	// Funcion encargada de verificar el ordenamiento de un vector de permutaciones.
	/*bool is_sort(vi &v, int sz){
		FOR(n, 1, sz){
	...
	// Funcion encargadad de calcular el minimo de combinaciones para ordenar un vector
	// de permutaciones.
	/*int Min_permutation(vi &permutation_index, vi &permutation){
		// Inicializacion de variables de trabajo.
		int left_x, right_y;
		int idx_x, idx_y;
		int prevTo_x, prevTo_y;
		int min_combination;
		int sz = permutation_index.size()-1;
 
		// Calculamos el valor X para el par inicial y su indice.
		left_x = (sz+1)/2;
		idx_x = permutation_index[left_x];
 
		// Calculamos el valor Y para el par inicial y su indice.
		right_y = (sz+2)/2;
		idx_y = permutation_index[right_y];
 
		// Verificar si la permutacion ya se encuentra ordenada.
		if(is_sort(permutation, sz)) return 0;
		
		// Iniciamos con la cantidad de combinaciones necesarias para obtener el vector
		// ordenado. Esto es cantidad de numeros dividido por el tamaño del par = 2;
		min_combination = (sz/2);
 
		// Para el caso de imparidad de N, recordemos que sobra un elemento que no esta 
		// en los pares combinatorios.
		if( sz % 2 != 0 ){
			// Verificacion de indices para el par inicial (n/2, n+2/2) tal que 
			// index(n/2) < index(n/2+1) < index(n/2+2)
			if( (permutation_index[left_x-1] < idx_x) && (idx_x < permutation_index[left_x+1]) ){
				// Movernos a la izquierda de x en los pares iniciales.
				left_x--; 
				idx_x = permutation_index[left_x];
 
				// Movernos a la iderecha de x en los pares iniciales.
				right_y++;
				idx_y = permutation_index[right_y];
 
			// Si no cumple index(n/2) < index(n/2+1) < index(n/2+2) para el par inicial
			// entonces es requerido el maximo de combinaciones para obtener el orden.
			} else {
				return min_combination;
			}
		}
 
		// Vamos cambiando de pares, de (x, y) a (x-1, y+1) y verificar por indices si
		// la solucion es valida hasta el punto actual, si se cumple que a nivel de indices
		// x-1 <= x ^ y <= y+1 entonces ya existe un par que no necesitamos mover probar
		// por tanto actualizamos los x e y y reducimos las minimas permutaciones.
		prevTo_x = idx_x; prevTo_y = idx_y;
		while( (idx_x <= prevTo_x) && (prevTo_y <= idx_y) && (prevTo_x < prevTo_y) ){
			// Reducir el minimo de combinaciones.
			min_combination--;
	...
	// Algoritmo de Kruskal apra determinar minimum Spanning Tree en base a pesos en arcos.
	// Se modifica el algoritmo para trabajar en baso a dos pesos en un mismo arco.
	lli Kruskal(){
		// Spanning Tree formado desde arcos con menor peso en oro.
		// Y luego sera reestructurado por el valor de la plata.
		va NewMST(n_nodes);
		
		int nodes_MST = 0;		  // Cantidad de nodos en el Spanning tree tal que esten conectados.
 
		lli min_cost = INF;		  // Minimo valor en el Spanning Tree inicialmente.
 
		lli max_silver;			  // Para cada arista en orden de costo de oro.
		
		for( auto arc : Edges ){
			max_silver = -1;	  // Maxima cantidad de plata encontrada hasta ahora.

			// Inicializar el vector de padres. Conjuntos singleton.
			FOR(n, 1, n_nodes) make_set(n);
			
			NewMST[nodes_MST++] = arc; // Sumar el arco el new MST.
 
			// Para el nuevo MST se debe ordenar los arcos en base al peso en plata.
			// Partiendo desde el mas reciente agregado hasta primero agregado.
			for(int n_arcs = nodes_MST-1; n_arcs > 0; n_arcs--){
				if(NewMST[n_arcs].s_cost < NewMST[n_arcs-1].s_cost)
					swap(NewMST[n_arcs], NewMST[n_arcs-1]);
			}
 
			// Buscamos por los arco actuales del newMST los valores de oro y plata que 
			// satisfaga la conficion de g_i <= a y s_i <= b del problema.
			int f_n, f_ady;
			int aux = 0; // Contador auxiliar de nodos en MST.
			FOR(i, 0, nodes_MST){
				// Buscamos padres.
				f_n = find_set(NewMST[i].n);
				f_ady = find_set(NewMST[i].ady);
				// En caso de ser diferentes los padres entonces se conecta con el 
				// MST y actulizamos el costo de plata.
				if( f_n != f_ady){
					union_sets(f_n, f_ady);
					max_silver = max(max_silver, NewMST[i].s_cost);
					NewMST[aux++] = NewMST[i];
				}
 
				// Verificar si el MST tiene todos los arcos.
				if(aux == n_nodes-1) break;
			}
 
			// Actualizamos el costo minimo total.
			if(aux == n_nodes-1) {
				lli min_gold = arc.g_cost*Gold;
				min_cost = min(min_cost, min_gold + max_silver*Silver);
			}
 
			// Actualizar la cantidad de nodos en el MST.
			nodes_MST = aux;
		}
 
		if(nodes_MST != n_nodes-1) return -1;
...
int main() {
	...
	// Lectura de nodos, arcos y pesos.
	cin >> N >> M;
	cin >> Gold >> Silver;
	Graph G(1, N);
	int xi, yi;
	...
	FOR(m, 1, M){
		cin >> xi >> yi >> gi >> si;
		G.add_arc_v2(xi, yi, gi, si);
	}

 	sort(G.Edges.begin(), G.Edges.end()); // Ordenar arcos en funcion del peso en oro.
 	// Aplicacion de Kruskal para determinar el oro y plata minimo necesario.
	cout << G.Kruskal() << endl;

/*
	...
	Carnet: 1710303
	Nick: JMLTUnderCode
	Problema: Codeforces Beta Round 22 (Div. 2 Only) E. Scheme

	** Idea General para el problema: **

	Se debe buscar los pares(arcos) tal que el grafo sea fuertemente conectado. Para esto
	realizamos un DFS por el arreglo input y como las caracteristicas del grafo es arcos
	dirigidos se tiene que cuando un llamada DFS sea a un nodo ya VISITADO entonces hemos 
	encontrado un nodo "terminal", esto quiere de que el camino que llevo a luego repetir
	un ancestro es el "final" de ese camino. Ej.
	...
	Notemos que al comenzar en nodo (1 --> 2 --> 3 --> 1) se tiene que 3 es el nodo "terminal"
	de ese camino, asi seteamos todos los elementos del camino con dicho nodo terminal.
	De la misma forma para los demas nodos, 7 apunta a 1 cuyo terminal es 3, lo mismo para
	5, 4 y 6 que terminan en el mismo 3. Esto mismo pasa si comentamos desde cualquier otro
	nodo del grafo, (5 --> 4 --> 3 --> 1 --> 2 --> 3) se tendria 2 como nodo terminal, etc.

	En otras palabras los candidatos(terminales) forman parte de una C.F.C. del grafo en si,
	si todo nodo llega a esa C.F.C. entonces realizar una conexion desde ella con todos 
	aquellos nodos del grafo tal que su grafo interior es 0(Nunca fueron informados de la noticia)
	forma una unica C.F.C. 

	Al rellenar estos arcos {(3,7), (3, 5), (3, 6)}, {(2,7), (2, 5), (2, 6)}, etc se tiene que 
	todo par de nodos posee un camino. 

	** Actualizacion de Libreria/Class: **

	1. Comentado/Descomentado en funcion Tarjan junto a variables/estructuras que usa la misma
...
public:
	...
	*/
	// Constructor DEFAULT.
	NODE(){
		n = 0; visited = 0; in_grade = 0;
		//ex_grade = 0; min_cost = 0;
...
const int MaxN = 1e5+5;		// Numero maximo de nodos.(BolgenOS community members)
int N;				// Numero de nodos.(Algura del risco)
int f_y;			// Llamado.
vi terminal_n(MaxN, 0);		// Vector de nodos terminales;
vi vis_term(MaxN, 0);		// Vector de visitados para nodos terminales.
vvi pairs(2);			// Vector para almacenar los matchs de nodos como arcos.
...
int main() {
	...
	Graph G(1, N);
	FOR1(k, N){
		cin >> f_y; G.add_arc_v1(k, f_y);
	}

	// Seteamos el/los nodos terminales para cada nodo.
	FOR1(n, N){
		if(!G.Nodes[n].visited) G.Tarjan(n);
	}

	// Busquemos los nodos con grado interno = 0 y lo emparejamos con tu terminal.
	// Luego se debe marcar la terminal como usaba para dicho nodo.
	pairs[1].pb(0);
	FOR1(k, N){
		if(!G.Nodes[k].in_grade){
			pairs[0].pb(terminal_n[k]);
			pairs[1].pb(k);
			vis_term[terminal_n[k]] = 1;
		}
	}

	// Comprobamos en caso de que existan mas de 1 C.F.C. dicho emparejamiento debe
	// considerarse para luego conectar C.F.C. entre si.
	FOR1(k, N){
		if(terminal_n[k] == k && !vis_term[k]){
			pairs[0].pb(k); pairs[1].pb(k);
		}
	}
	pairs[1][0] = pairs[1][pairs[1].size()-1];

	// En caso de solo tener un arco seteado y este tenga misma terminal quiere decir
	// que todo nodo pudo llegar a el y viceversa por tanto se tiene 1 unica C.F.C.
	int count = pairs[0].size();
	if(count == 1 && pairs[0][0] == pairs[1][0]) count = 0;

	// Se deben mostrar los pares rotados 1 unidad, recordemos que pueden haber mas de
	// 1 C.F.C. por tanto el nodo terminal para cada componente no tiene relacion.
	cout << count << endl;
	FOR0(k, count){
		cout << pairs[0][k] << " " << pairs[1][k] << endl;
	}

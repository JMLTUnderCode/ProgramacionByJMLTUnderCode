/*
	...
	** Actualizacion de Libreria/Class: **
	1. Insercion de funcion "Floyd_Warshall_m", la version original esta diseñada para determinar
	las distancias desde un vector de arcos dado. El problema otorga directamente la matriz de 
	distancias.
	2. Ligera modificacion en class Graph, se setea por default el costo minimo de un nodo a 0.
...
const int MaxN = 505;		// Numero maximo de nodos.
int N;				// Numero de nodos.
vi node_del(MaxN, 0);		// Vector de nodos a eliminados.
...
int main() {
	...
	Graph G(1, N);
	FOR1(i, N) {
		FOR1(j, N) cin >> G.dist[i][j];
	}
	
	FOR1(i, N) cin >> node_del[i];
	...
	FOR1(k, N) cout << G.Nodes[k].min_cost << " ";
	cout << endl;

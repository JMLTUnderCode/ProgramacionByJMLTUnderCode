/*
	...
	Carnet: 1710303
	Nick: JMLTUnderCode
	Problema: Educational Codeforces Round 142 (Rated for Div. 2) C. Min Max Sort

	** Idea General para el problema: **
	
	Para obtener el minimo de movimientos en el interno de ordenar una vector de permutaciones
	se debe aplicar el concepto de "pares iniciales" como los movimientos que se deben ejecutar
	en este problema. Es decir, el par (x, y) es un par de elementos que se quitaron y de la 
	permutacion y el minimo(x, y) pasa a la primera posicion y el max(x , y) pasa al ultimo.
	Se tienen dos casos, cuando N es par y cuando N es impar siendo N el numero de elementos
	en el vector de permutacion. Entonces

	N Par: Se tiene los pares iniciales minimo tal que el vector quede ordenado.
	 (  1 ,  N   )  Ej.  N = 10 -> ( 1 , 10)
	 (  2 ,  N-1 )                 ( 2 , 9 )
	 (  3 ,  N-2 )                 ( 3 , 8 )
	    . , ,                      ( 4 , 7 )
	    . , .                      ( 5 , 6 )
	 ( N/2, N/2+1)
	Aplicando en orden desde (5, 6) hasta (1,10) el vector queda ordenado de forma garantizada.

	N Impar: Se tiene los pares iniciales minimo tal que el vector quede ordenado.
	 (  1 ,  N   )  Ej.  N = 9  -> ( 1 , 9 )
	 (  2 ,  N-1 )                 ( 2 , 8 )
	 (  3 ,  N-2 )                 ( 3 , 7 )
	    . , ,                      ( 4 , 6 )
	    . , .                      
	 ( N/2, N/2+2)
	De la misma forma, partiendo de (4, 6) hasta (1, 9) se ordena un vector de length N garantizado.
	Por tanto, la solucion propuesta inicia con este primer par ( N/2, N/2+1) o ( N/2, N/2+2) segun
	sea el caso de paridad. Y se va verificando que exista un ordenamiento posterior mediante indices
	dado que una permutacion [1, 3, 5 ,6, 4, 2] tiene como vector de indices que
		
	index_perm   [1, 6, 2, 5, 3, 4]
	index_vec ->  1  2, 3, 4, 5, 6   <- permutation_value
	
	El numero 1 (index) se leyó  de 1.
	El numero 3 se leyó de 2.
	El numero 5 se leyó de 3.
	El numero 6 se leyó de 4. 
	... etc
	
	** Actualizacion de Libreria/Class: **
	1. Inclusion de dos funciones para determinar la cantidad de minimas combinaciones en 
	el cambio de una permutacion para ordenarlas.
		- is_sort: Verifica que un vector esta ordenado por valor == indice creciente.
		- Min_permutation: Determina las minimas permutaciones/cambios en una vector
		de permutaciones para ordenar el vector.
	2. Ligera modificacion en FastIO_RWFile(), se cambio de 0 a NULL en "tie".
...
int main() {
	...
	while(T--){
		cin >> N;
		vi permutation_index(N+1, 0);
		Graph G(1, N+1);

		// Se guarda el indice por numero leido de la permutacion.
		FOR(n, 1, N) {
			cin >> permutation[n];
			permutation_index[permutation[n]] = n;
		}

		// Calculo de minimas permutaciones para ordenar el vector.
		cout << G.Min_permutation(permutation_index, permutation) << endl;
	}

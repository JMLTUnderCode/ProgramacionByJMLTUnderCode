/*
	...
	Carnet: 1710303
	Nick: JMLTUnderCode
	Problema: Codeforces Round #736 (Div. 2) B. Gregor and the Pawn Game

	** Idea General para el problema: **
	
	La solucion a este problema es lograr entender los casos en que un peon puede llegar a la
	fila enemiga o no.
	Caso 1: Para el peon gregor en posicion columna j existe una posicion en 0 en la fila 
	enemiga columna j entonces existe un arco.
	Caso 2: Para el peon gregor en posicion columna j existe un peon enemigo en la fila enemiga
	columna j, entonces se verificar DOS CASOS:
		-Caso 2.1: En la fila enemiga columna j-1 existe un peon enemigo, entonces peon gregor
		puede llegar a la fila enemiga derrotando al peon enemigo. Existe arco.
		-Caso 2.2: En la fila enemiga columna j+1 existe un peon enemigo, entonces peon gregon
		puede llegar a la fila enemiga derrotando al peon enemigo. Existe arco.
	
	De este forma, se ha construido un grafo bipartito tal que al aplicar el algoritmo
	Hopcroft-Karp obtendremos el maximum Matching para este grafo lo que nos resolvera el problema
	retornando la mejor combiacion de "movimientos" para que la fila de Gregor llegue a la fila
	enemiga.
...
public:
	...
	*/
	...
	// Constructor DEFAULT.
	NODE(){
		n = 0; visited = 0; level = 0;
		//range = 0; in_grade = 0; ex_grade = 0; min_cost = 0;
...
int T;			// Numero de pruebas.
int N;			// Tamaño del tablero.
string pawn_enemy; 	// String de digitos binarios para los peones enemigos.
string pawn_gregor;	// Strgng de digitos binarios para los peones de Gregor.
...
int main() {
	...
	FOR(n, 1, T){
		...
		FOR(i, 1, N){
			// Existencia de un peon de gregor.
			if(pawn_gregor[i-1] != '1') continue;

			// Si en la posicion directa enemiga esta vacia entonces un arco directo.
			if(pawn_enemy[i-1] == '0') {
				G.add_arc_v1(i, i+N); continue;
			}

			// Si en la posicion izquierda enemiga.
			if ( (i-1)-1 > -1){
				// Existe un peon enemigo, entonces arco directo.
				if(pawn_enemy[i-2] == '1') G.add_arc_v1(i, (i-1)+N);
			}

			// Si en la posicion derecha enemiga.
			if ( i < N ) {
				// Existe un peon enemigo, entonces arco directo.
				if(pawn_enemy[i] == '1') G.add_arc_v1(i, i+1+N);
			}
		}

		// Aplicamos el Maximum Matching.
		cout << G.Hopcroft_Karp() << endl;
	}
